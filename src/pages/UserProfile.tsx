...
const UserProfile = () => {
  ...
  const handleStartChat = async () => {
    ...
    try {
      const chatRoom = await createChatRoom(currentUser.uid, userId);
            ...
            <div className="relative h-48 bg-gray-200">
              ...
              {!isOwnProfile && (
                <button
                  ...
                  className="absolute top-4 right-4 bg-white px-4 py-2 rounded-full shadow hover:bg-gray-100 flex items-center"
                >
                  <ChatBubbleLeftRightIcon className="h-5 w-5 text-indigo-600 mr-1" />
                  <span className="text-sm font-medium text-indigo-600">Chat</span>
                </button>
              )}
            </div>
